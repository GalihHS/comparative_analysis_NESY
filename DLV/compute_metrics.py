...
def compute_rank(test_triple, graph_triples, total_nodes):
    """Compute the rank of a test triple based on the given graph for both head and tail prediction."""
    id1, relation, id2 = test_triple
    
    # Tail prediction (predicting id2 given id1 and relation)
    found_tail_links = sum(1 for (s, p, o) in graph_triples if s == id1 and p == relation)
    ...
    found_head_links = sum(1 for (s, p, o) in graph_triples if o == id2 and p == relation)
    if (id1, relation, id2) in graph_triples:
...
def compute_metrics(test_file, graph_file):
    ...
    for triple in test_triples:
        rank = compute_rank(triple, graph_triples, total_nodes)
        ranks.append(rank)
...
if __name__ == "__main__":
    ...
    metrics = compute_metrics(args.test_file, args.saturated_file)
