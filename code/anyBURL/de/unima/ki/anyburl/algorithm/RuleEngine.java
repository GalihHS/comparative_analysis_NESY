...
public class RuleEngine {
	...
	public static void applyRulesARX(LinkedList<Rule> rules, String path_sat,TripleSet testSet, TripleSet trainingSet, TripleSet validationSet, int k, PrintWriter resultsWriter) {
		...
		for (int threadCounter = 0; threadCounter < Settings.WORKER_THREADS; threadCounter++) {
			...
			predictors[threadCounter] = new Predictor(path_sat, testSet, trainingSet, validationSet, k, relation2Rules4Prediction);
	...
	*/
	...
	public static int predictMax(String path_sat, TripleSet testSet, TripleSet trainingSet, TripleSet validationSet, int k, HashMap<String, ArrayList<Rule>> relation2Rules4Prediction, Triple triple) {
		...
		result = writeTopKCandidates(path_sat, triple, testSet, kHeadCandidates, kTailCandidates, predictionsWriter, k);
		return result;
	}
	...
	*/
	...
	/*public static void predictNoisyOr(TripleSet testSet, TripleSet trainingSet, TripleSet validationSet, int k, HashMap<String, ArrayList<Rule>> relation2Rules, Triple triple) {
		...
		int result;
		result = writeTopKCandidates("data/family_small/train_sat.txt", triple, testSet, kHeadCandidates, kTailCandidates, predictionsWriter, k); //Je n'ai pas compris où c'était utile
	} */
	...
	*/
	...
	private static synchronized int writeTopKCandidates(String path_sat, Triple t, TripleSet testSet, LinkedHashMap<String, Double> kHeadCandidates, LinkedHashMap<String, Double> kTailCandidates, PrintWriter writer, int k) {
		writer.println(t);
		int result_right = 0;
		int result_wrong = 0;
		int result = 0;
		int i = 0;
		//String path_sat = "data/family_small/train_sat.txt";
		writer.print("Heads: ");
		for (Entry<String, Double> entry : kHeadCandidates.entrySet()) {
			if (t.getHead().equals(entry.getKey()) || !testSet.isTrue(entry.getKey(), t.getRelation(), t.getTail())) {
				...
				//metric for head (entry.getKey(), t.getRelation(), t.getTail()); }
				if (i == 0 && !path_sat.equals("NULL")) { //to test Contradicts@k
					boolean found = false;
					boolean found_good = false;
					try {
						BufferedReader reader = new BufferedReader(new FileReader(path_sat));
						String line;
						while ((line = reader.readLine()) != null) {
							String[] parts = line.split("\t");
							String head_sat = parts[0];
							String relation_sat = parts[1];
							String tail_sat = parts[2];
							if (head_sat.equals(entry.getKey()) && tail_sat.equals(t.getTail())){
								found = true;
								if (relation_sat.equals(t.getRelation())) {found_good = true;}
							}
						}
						reader.close();
					} catch (IOException e) {
					...
					if (!found || found_good) {result_right = result_right + 1;} else {result_wrong = result_wrong + 1;}
				}
				i++;
			}
			if (i == k) break;
		...
		for (Entry<String, Double> entry : kTailCandidates.entrySet()) {
			if (t.getTail().equals(entry.getKey()) || !testSet.isTrue(t.getHead(), t.getRelation(), entry.getKey())) {
				writer.print(entry.getKey() + "\t" + entry.getValue() + "\t");
				//metric for tail (t.getHead(), t.getRelation(), entry.getKey()); }
				if (i == 0 && !path_sat.equals("NULL")) {
					boolean found = false;
					boolean found_good = false;
					try {
						BufferedReader reader = new BufferedReader(new FileReader(path_sat));
						...
						while ((line = reader.readLine()) != null) {
							...
							String relation_sat = parts[1];
							String tail_sat = parts[2];
							if (head_sat.equals(t.getHead()) && tail_sat.equals(entry.getKey()) ){
								found = true;
								if (relation_sat.equals(t.getRelation())) {found_good = true;}
							}
						}
		...
		if (result_right + result_wrong == 2) {result = result_right;}
		if (result_right + result_wrong == 1) {result = result_right + 1;}
		if (result_right + result_wrong == 0) {result = 2;}
		if (result_right + result_wrong > 2) {System.out.println("ERROOOOORRRRRR : " + result_right + " - " + result_wrong);}
		return result;
