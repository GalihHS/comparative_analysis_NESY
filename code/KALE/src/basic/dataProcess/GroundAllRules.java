		...
		if (!fnRules.equals("")) {
			...
			while ((line = reader.readLine()) != null) {
				if (line.split("&&").length == 1 && line.endsWith("(x,y)")){
				    ...
				    String[] tokens = StringSplitter.RemoveEmptyEntries(StringSplitter
						.split("=>& ", line));
				    int iFstRelation = MapRelationID.getOrDefault(tokens[0],-1);
				    if (iFstRelation == -1) { System.err.println("Relation not found: " + tokens[0]); }
				    int iSndRelation = MapRelationID.get(tokens[1]);
...
//			Integer iSndRelation = LstRuleTypeII.get(iFstRelation);		
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length != 3) {
			System.err.println("please define the dataset folder, wn18 or fb122 or family, and the rule file name and the output (grounding.txt) file name");
			System.exit(1);
		}

		// Get the dataset folder from the arguments
		String datasetFolder = args[0];
		String rule_file = args[1];
		String groundings_file = args[2];


		int iEntities = 14951; //I don't think this is used
		// Input file:
		String fnRelationIDMap = "datasets/" + datasetFolder + "/relationid.txt";
		String fnRules = "datasets/" + datasetFolder + "/" + rule_file;
		String fnTrainingTriples = "datasets/" + datasetFolder + "/train.txt";
		//Output file:
        String fnOutput = "datasets/" + datasetFolder + "/" + groundings_file;
