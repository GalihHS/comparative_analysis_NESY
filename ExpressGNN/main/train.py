...
def train(cmd_args):
  ...
  if cmd_args.load_method == 1:
    ...
    for current_epoch in range(cmd_args.num_epochs):
      ...
      with torch.no_grad():
        ...
        if is_current_best:
          ...
          torch.save(gcn.state_dict(), joinpath(savepath, 'gcn.model'), _use_new_zipfile_serialization=False)
    ...
    os.system('mv %s %s' % (logpath, joinpath(cmd_args.exp_path,
                                              'performance_hits@10_%.4f_mmr_%.4f.txt' % ((hits10 / cnt), (rrank / cnt)))))
    os.system('mv %s %s' % (logpath, joinpath(cmd_args.exp_path,
                                              'performance_hits@3_%.4f_mmr_%.4f.txt' % ((hits3 / cnt), (rrank / cnt)))))
    os.system('mv %s %s' % (logpath, joinpath(cmd_args.exp_path,
                                              'performance_hits@1_%.4f_mmr_%.4f.txt' % ((hits1 / cnt), (rrank / cnt)))))
    pbar.close()
    print(time.ctime())
