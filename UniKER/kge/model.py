...
class KGEModel(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma, partial_init,
        ...
        if partial_init:
            self.entity_embedding = nn.Parameter(torch.Tensor(entity_embedding), requires_grad=True)
            self.relation_embedding = nn.Parameter(torch.Tensor(relation_embedding), requires_grad=True)
    ...
    @staticmethod
    def test_step(model, test_triples, all_true_triples, args, train_triples):
        '''
        ...
        if args.countries:
                ...
                for test_dataset in test_dataset_list:
                    for positive_sample, negative_sample, filter_bias, mode in test_dataset:
                        ...
                        for i in range(batch_size):
                            ...
                            # ------------------  new metric ---------------------
                            metric_dataset = ""
                            if args.data_path == "./data/family_small/":
                                metric_dataset = "family_small"
                            if args.data_path == "./data/family_medium/":
                                metric_dataset = "family_medium"
                            if metric_dataset == "family_small" or metric_dataset == "family_medium": #if we compute the metric
                                if mode == "tail-batch":
                                    start_node = positive_sample[i][0].item()
                                    end_node = argsort[i][0].item()
                                    if start_node == end_node:
                                        end_node = argsort[i][1].item()
                                    node_id = end_node
                                elif mode == "head-batch":
                                    start_node = argsort[i][0].item()
                                    end_node = positive_sample[i][2].item()
                                    if start_node == end_node:
                                        start_node = argsort[i][1].item()
                                    node_id = start_node
                                ...
                                with open("data/"+metric_dataset+"/entities.dict", 'r') as entities_file:
                                    for line in entities_file:
                                        elems = line.strip().split("\t")
                                        entity_dict[int(elems[0])] = int(elems[1])
                                with open("data/"+metric_dataset+"/train_sat.txt", 'r') as train_sat:
                                    for line in train_sat:
                                        elems = line.strip().split("\t")
                                        if entity_dict[start_node] == int(elems[0]) and entity_dict[end_node] == int(elems[2]):
                                            found = True
                                            if rel_id == dicttttt[elems[1]]:
                                                found_true = True
                                if found_true:
                                    new_metric[0] += 1 
                                elif found:
            ...
            print("new metric is: ", new_metric) #[0,0,0] if metric not computed
    ...
    @staticmethod
    def evaluate_step(model, test_triples, id2entity, id2relation, args):
        '''
