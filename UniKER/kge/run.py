...
def parse_args(args=None):
    ...
    parser.add_argument('--partial_init', action='store_true')
...
def main(args):
    ...
    if args.partial_init:
        entity_embedding = np.load(os.path.join(args.save_path, 'entity_embedding.npy'))
        relation_embedding = np.load(os.path.join(args.save_path, 'relation_embedding.npy'))
        ...
        kge_model = KGEModel(
            ...
            gamma=args.gamma,
            partial_init=args.partial_init,
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            if args.do_valid and step % args.valid_steps == 0:
                ...
                metrics = kge_model.test_step(kge_model, valid_triples, all_true_triples, args, train_triples)
    ...
    if args.do_test:
        ...
        metrics = kge_model.test_step(kge_model, test_triples, all_true_triples, args, train_triples)
        log_metrics('Test', step, metrics)

    if args.evaluate_train:
        logging.info('Evaluating on Training Dataset...')
        metrics = kge_model.test_step(kge_model, train_triples, all_true_triples, args, train_triples)
        log_metrics('Test', step, metrics)
